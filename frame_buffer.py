...
class frameBuffer():
	...
	def insertData(self, buffer, x_orig, y_orig, data):
		x_size = len(data[0])
		y_size = len(data)
		#print(y_orig, x_orig)
		#print(y_size, x_size)
	...
	def insertStrBufWithRoll(self, buffer, x_orig, y_orig, data, xshift):
		...
		for y in range(y_size):
			for x in range(buf_xsize):
				if(x<img_xsize):
					buffer[y_orig + y][x_orig + x] = data[y][x+xshift]
				else:
	...
	def updateStringBuffer(self, buffer, row):
		...
		for i in range(len(self.string[row])):
			...
			self.insertData(buffer, x, y, char_data)
			self.insertData(buffer, x+5, y, self.font.get_gap())
			x += 6
		#print(self.fb)
		return buffer
	...
	def writeString(self, string, row):
		...
		for i in range(len(string)):
			char_wid, char_data = self.font.get_char(string[i])
			self.sb[row] = self.insertData(self.sb[row], currx, 0, char_data)
			currx += char_wid + 1
	...
	def drawPixel(self, x, y):
		if(self.fb[y][x]):
			if(y%2 == 0):
				...
				self.strip.setPixelColor((y*26)+x, Color(0,64,0))
			else:
				#self.strip.setPixelColor((y*26)+(25-x), Color(32,32,32))
				self.strip.setPixelColor((y*26)+(25-x), Color(0,64,0))
		else:
			if(y%2 == 0):
				#self.strip.setPixelColor(y*26+x, Color(0,0,0))
				self.strip.setPixelColor(y*26+x, Color(32,0,0))
			else:
				#self.strip.setPixelColor(y*26+(25-x), Color(0,0,0))
				self.strip.setPixelColor(y*26+(25-x), Color(32,0,0))
